...
@ad_routes.route("/api/ads", methods=["POST"])
@jwt_required()
def create_ad():
    data = request.json
...
# Public: View All Ads
@ad_routes.route("/api/ads", methods=["GET"])
def get_ads():
    ads = Ad.query.all()
    ads_list = [
        {
...
#Edit ad
@ad_routes.route("/api/ads/<int:ad_id>", methods=["PUT"]) 
@jwt_required()
def edit_ad(ad_id):
...
#Delete ad
@ad_routes.route("/api/ads/<int:ad_id>", methods=["DELETE"])
@jwt_required()
def delete_ad(ad_id):
    user_id = get_jwt_identity()  # Get logged-in user ID
    ad = Ad.query.get(ad_id)
...
@ad_routes.route("/api/my-ads", methods=["GET"])
@jwt_required()
def get_my_ads():
    user_id = get_jwt_identity()  # Get logged-in user ID
